...
void setup() { 
  ...
  //Inicia la camara en alta calidad para reservar memoria
  if(psramFound()){
    config.frame_size = FRAMESIZE_UXGA;
    config.jpeg_quality = 10;  //Compresion del jpeg
    config.fb_count = 2;
  } else {
    config.frame_size = FRAMESIZE_SVGA;
    config.jpeg_quality = 12;  //Compresion del jpeg
    config.fb_count = 1;
  }
  // camera init
...
void loop() { 
  ...
  if (sensor == HIGH){
    Serial.println("Tomando captura...");
    String test = parseString(Photo2Base64());
    String imagen = "{\"photo\":\""+test+"\"}";
    json.setJsonData(imagen);
